
import 'package:flutter/material.dart';

class AllColors{
  Color stepperBackgroundColor = const Color(0xff3c70f3);
  Color selectedStepperBackgroundColor = const Color(0xff67aa51);
  Color notSelectedStepperBackgroundColor = const Color(0xffe8edee);
  Color nextButtonBackgroundColor = const Color(0xff6f9cef);
  Color labelColor = const Color(0xffd4d4d4);
  Color backgroundColor = const Color(0xfff9f9f9);
  Color finansTextColor = const Color(0xff3f6efa);
  Color whiteColor = Colors.white;
  Color blackColor = Colors.black;

}